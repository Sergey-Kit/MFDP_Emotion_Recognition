...
Данная ветка для исследования включает в себя скрипт (emotion.py) по обработке одного изображения с хоста с указанием пути изображения. В результате получаем эмоцию на лице. Также есть скрипт (emotion_loop.py) для обработки потока изображений из интернета посредством HTTP запросов на адреса указанные в Excel-файле (из датасета EmotioNET)
